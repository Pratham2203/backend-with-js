...
const loginUser=asyncHandler(async (req,res)=>{
    ...
    const options={
        httpOnly:true,              //so that cookies can be modified by servers only and not by frontend
        secure:true
    }
...
const refreshAccessToken=asyncHandler(async (req,res)=>{
    const incomingRefershtoken=req.cookies.refershToken || req.body.refreshToken
    ...
    const channel=await User.aggregate([
        ...
        },
        {
            $addFields:{
                ...
                isSubscribed:{
                    $cond:{
                        if:{$in:[req.user?._id,"$subscribers.subscriber"]},  //check if you(user) is 'in' subscribers list of that channel or not
                        then:true,
                        else:false
                    }
                }
...
const getWatchHistory=asyncHandler(async (req,res)=>{
    ...
    return res
    .status(200)
    .json(new ApiResponse(200,user[0].getWatchHistory,"watch history retrieved successfully"))
})
export {registerUser,loginUser,logoutUser,refreshAccessToken,changeCurrentPassword,updateAccountDetails,getCurrentUser,updateUserAvatar,getWatchHistory,getUserChannelProfile,updateUserCoverImage}
